<template>
    <div>
        <div class="new-comdetails">
            <div class="new-one new-comps" v-for="(item,index) in product.data" @mouseover="newTrends(index)" @mouseout="newTrendsout(index)" :class="{'hover-for':ind === index }">
                <nuxt-link :to="'/plase/'+item.id + '?type=1'">
                	<div class="img-end">
                		<img class="img-end" :src="item.image">
                	</div>
                    <div class="h-btm clearfix">
                        <div class="leftNewst" :class="{'hover-one':ind === index}">
...
export default {
  ...
  head () {
          return {
		      title: '多多达人机器人-新闻动态',
		       meta: [
		        { hid: 'new-newsTrends', name: 'new-newsTrends', content: '多多达人机器人-新闻动态' }
		      ]	      
		    }
		  },
  computed: {
